...
(** [doc_cmd] clean files generated by [cs3110 doc] *)
let doc_cmd (cfg : Cli_config.t) : string    =
  Format.sprintf "rm -rf %s"
    cfg.doc.output_directory
...
let all_cmd (cfg : Cli_config.t) : string       =
  Format.sprintf "%s; rm -rf %s"
    (compile_cmd cfg)
    Cli_config.cOUTPUT_DIRECTORY
