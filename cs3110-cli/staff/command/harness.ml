...
(** [harness_run_test o d t] Run test [t] on the submission in dir [d]. Collect
...
let command =
  Command.basic
    ...
    (fun v tests ps release qc test_dir output_dir sheet_location subs () ->
      ...
      let release_dir = Option.value release ~default:cfg.harness.input_directory in
      let ()          = assert_file_exists ~msg:"Release directory missing" release_dir in
      let test_files  = get_test_files cfg tests in
      let opts = {
        ...
        spreadsheet_location = Option.value sheet_location ~default:cfg.harness.output_spreadsheet;
