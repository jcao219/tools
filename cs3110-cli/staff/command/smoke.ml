...
(** [copy_directory o d] save a copy of the directory [dir] for later diffing.
...
let copy_directory (opts : options) (netid : string) (dir : string) : unit =
  ...
  let () = ignore (Sys.command (Format.sprintf "rm -rf %s" existing_dir)) in
...
let get_smoke_targets () : string list =
