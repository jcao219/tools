...
let run (directories : string list) : unit =
  ...
  * name of a previous no-compile *)
  ...
  let results_chn = open_out cfg.diff.output_spreadsheet in
  ...
  (* Close up *)
  let () = ignore(Sys.command (Format.sprintf "rm -f %s" cfg.diff.temporary_file)) in
  let () = close_out results_chn in
  Format.printf "Finished inputting decisions for %d files. See '%s' for results\n%!" (!num_files_diffed) cfg.diff.output_spreadsheet
