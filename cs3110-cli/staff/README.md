...
* `cs3110-staff clean` remove the files generated through compilation.
* `cs3110-staff compile <filename>` compile the target file, generating corresponding `.cmo` and `.cmi` files in the `_build` directory.
* `cs3110-staff cms <filename>` create a CMS-ready spreadsheet from the sheet `filename`. Adds comments from `./output`.
* `cs3110-staff diff <dir_names>` compares the files stored in the directory `_nocompile` with the files in the supplied `dir_names`.
* `cs3110-staff doc [src_dir] <output_dir>` generates [ocamldoc](http://caml.inria.fr/pub/docs/manual-ocaml-400/manual029.html) documentation from the `.mli` files in `src_dir` and dumps the generated html into `output_dir`. The default `src_dir` is the current working directory.
* `cs3110-staff email` Send all the emails generated by running the smoke test.
* `cs3110-staff harness <dir_names>` Runs the tests stored in `./tests` within each directory of the supplied `dir_names`.
* `cs3110-staff help` print a quick summary of the available commands.
* `cs3110-staff run <filename> [args]` Run the executable created by `cs3110-staff compile <filename>` with the supplied arguments.
* `cs3110-staff smoke <dir_names>` Compile many files within each dir of `dir_names`. Files to compile are determined by the file `./smoke_test`.
* `cs3110-staff test <filename>` Execute the `pa_ounit` tests within the file.
...
The staff tool lets us run a smoke test, send emails to students who failed to compile, diff previous no-compiles with updated submissions, run a full test harness, and generate a CMS-ready spreadsheet.
...
The smoke test uses `cs3110-staff compile` for each file to make sure that students' code compiles.
...
`cs3110-staff smoke` runs as follows:

* Input is a list of directories. Each should be named with a valid Cornell netid. Example: `cs3110-staff smoke Submissions/blg59`.
...
`cs3110-staff smoke Submission/blg59` tests a single student, `blg59`, and `cs3110-staff smoke Submission/*` tests every student.
...
`cs3110-staff email` sends email messages to students.
...
Call `cs3110-staff email`. No arguments!
...
`cs3110-staff diff` compares old and new submissions.
...
`cs3110-staff diff Submissions/blg59` behaves as follows:
...
`cs3110-staff diff Submissions/*` is most likely the desired behavior; it works just the same as the arguments to the smoke test.
...
* Check that a rubric exists. If not, call `cs3110-staff rubric` on a compiling student submission.
...
Call `cs3110-staff harness <targets>` with the student submissions directories.
...
`cs3110-staff cms <filename>` generates a spreadsheet ready for upload to CMS.

#### Implementation ####

Iterates over the spreadsheet given as its argument and collects data from CAPITALIZED columns.
So a column named 'zardoz' will be ignored, but columns 'Zardoz', 'ZARDOZ', and 'ZardoZ' will be collected.
This is because CMS requires capitalized columns -- make sure the input is properly formatted!

Additionally puts comments from the output directory (if it exists) on students' columns in the sheet.

#### Usage ####

Once you've made and properly validated a spreadsheet, run `cs3110-staff cms <filename>`, replacing `filename` with the name of your sheet.
...
The past is littered with good ideas.
